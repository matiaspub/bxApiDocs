...
class CAllSalePaySystem
{
	...
	/**
	* <p>Функция возвращает параметры платежной системы с кодом ID. Если установлен параметр PERSON_TYPE_ID, то возвращаются так же параметры соответствующего обработчика платежной системы.</p>
	*
	*
	*
	*
	...
	* системы (возвращается, если в функцию передается тип плательщика)
	* </td> </tr> <tr> <td>PSA_NAME</td> <td>Название обработчика (возвращается, если в
	* функцию передается тип плательщика)</td> </tr> <tr> <td>PSA_ACTION_FILE</td>
	* <td>Скрипт обработчика (возвращается, если в функцию передается
	* тип плательщика)</td> </tr> <tr> <td>PSA_RESULT_FILE</td> <td>Скрипт запроса
	* результатов (возвращается, если в функцию передается тип
	* плательщика)</td> </tr> <tr> <td>PSA_NEW_WINDOW</td> <td>Флаг (Y/N) открывать ли скрипт
	* обработчика в новом окне (возвращается, если в функцию передается
	* тип плательщика)</td> </tr> </table> <p>  </p<a name="examples"></a>
	*
	*
	* <h4>Example</h4> 
	* <pre>
	...
	*/
	...
	/**
	* <p>Функция обновляет параметры платежной системы с кодом ID в соответствии со значениями из массива arFields </p>
	*
	*
	*
	*
	...
	*/
	...
	/**
	* <p>Функция удаляет платежную систему с кодом ID. Если к платежной системе с кодом ID привязаны заказы, то эта платежная система удалена не будет.</p>
	*
	*
	*
	*
