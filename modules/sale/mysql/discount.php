...
class CSaleDiscount extends CAllSaleDiscount
{
	...
	/**
	* <p>Метод добавляет новую скидку на сумму заказа с параметрами из массива <i> arFields</i>. Нестатический метод.</p>
	*
	*
	...
	* предоставляется эта скидка;</li> 			<li> <b>CURRENCY</b> - валюта денежных
	* полей в записи;</li> 			<li> <b>DISCOUNT_VALUE</b> - величина скидки;</li> 			<li>
	* <b>DISCOUNT_TYPE</b> - тип величины скидки (P - величина задана в процентах, V
	* - величина задана в абсолютной 				сумме);</li> 			<li> <b>ACTIVE</b> - флаг (Y/N)
	* активности скидки;</li> 			<li> <b>SORT</b> - индекс сортировки (если по
	...
	*/
	...
	/**
	* <p>Метод обновляет параметры скидки с кодом ID на  параметры из массива arFields. Нестатический метод. </p>
	*
	*
	...
	* к которому привязана эта скидка;</li> 			<li> <b>PRICE_FROM</b> - общая
	* стоимость заказа, начиная с которой предоставляется эта
	* скидка;</li> 			<li> <b>PRICE_TO</b> - общая стоимость заказа, до достижения
	* которой предоставляется эта скидка;</li> 			<li> <b>CURRENCY</b> - валюта
	* денежных полей в записи;</li> 			<li> <b>DISCOUNT_VALUE</b> - величина скидки;</li>
	* 			<li> <b>DISCOUNT_TYPE</b> - тип величины скидки (P - величина задана в
	* процентах, V - величина задана в абсолютной 				сумме);</li> 			<li>
	* <b>ACTIVE</b> - флаг (Y/N) активности скидки;</li> 			<li> <b>SORT</b> - индекс
	* сортировки (если по сумме заказа доступно несколько скидок, то
	* берется первая по сортировке)</li> 		</ul>
	*
	...
	*/
	...
	/**
	* <p>Метод возвращает результат выборки записей из скидок на заказ в соответствии со своими параметрами. Нестатический метод.</p> <p>Метод устарел, вместо него рекомендуется использовать <code>\Bitrix\Sale\Internals\DiscountTable::getList</code>.</p>
	*
	*
	...
	* находится в передаваемом в фильтр разделенном запятой списке
	* значений;</li> 			<li> <b>~</b>  - значение поля проверяется на
	...
	* фильтра: 		<pre class="syntax">array("!CURRENCY" =&gt; "USD")</pre> 		Этот фильтр означает
	* "выбрать все записи, в которых значение в поле CURRENCY (валюта) не
	* равно USD".<br><br> 	Значение по умолчанию - пустой массив array() -
	* означает, что результат отфильтрован не будет.
	*
	* @param array $arGroupBy = false Массив полей, по которым группируются записи 		скидок. Массив
	...
	* width="15%">Ключ</th>     <th>Описание</th>   </tr> <tr> <td>ID</td>     <td>Код скидки.</td> </tr>
	* <tr> <td>LID</td>     <td>Код сайта, к которому привязана эта скидка.</td>   </tr>
	* <tr> <td>PRICE_FROM</td>     <td>Общая стоимость заказа, начиная с которой
	* предоставляется эта скидка.</td>   </tr> <tr> <td>PRICE_TO</td>     <td>Общая
	* стоимость заказа, до достижения которой предоставляется эта
	* скидка.</td>   </tr> <tr> <td>CURRENCY</td>     <td>Валюта денежных полей в записи.</td>
	*   </tr> <tr> <td>DISCOUNT_VALUE</td>     <td>Величина скидки.</td>   </tr> <tr> <td>DISCOUNT_TYPE</td>    
	* <td>Тип величины скидки (P - величина задана в процентах, V - величина
	* задана в абсолютной сумме).</td>   </tr> <tr> <td>ACTIVE</td>     <td>Флаг (Y/N)
	* активности скидки.</td> </tr> <tr> <td>SORT</td>     <td>Индекс сортировки (если
	* по сумме заказа доступно несколько скидок, то берется первая по
	* сортировке).</td>   </tr> <tr> <td>USER_GROUPS</td>     <td>Перечень групп
	* пользователей, на которые должна действовать скидка.</td>   </tr>
