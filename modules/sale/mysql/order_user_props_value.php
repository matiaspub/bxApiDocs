...
class CSaleOrderUserPropsValue extends CAllSaleOrderUserPropsValue
{
	...
	/**
	* <p>Метод возвращает результат выборки записей из значений свойств профилей покупателя в соответствии со своими параметрами. Нестатический метод.</p>
	*
	*
	...
	* запятой списке значений. Можно просто передавать массив
	...
	* фильтра: 		<pre class="syntax">array("USER_PROPS_ID" =&gt; 120)</pre> 		Этот фильтр означает
	* "выбрать все записи, в которых значение в поле USER_PROPS_ID (код профиля
	* покупателя) равно 120".<br><br> 	Значение по умолчанию - пустой массив
	* array() - означает, что результат отфильтрован не будет. <br><br><div
	* class="note"> <b>Важно!</b> Если результат фильтровать не нужно, то пустой
	* массив фильтра надо передать обязательно. Это требуется из-за
	* наследия старого API.</div>
	*
	...
	* стоять любое поле 		значений свойств профилей покупателя. В
	...
	* width="15%">Ключ</th>     <th>Описание</th>   </tr> <tr> <td>ID</td>     <td>Код свойства
	* профиля покупателя.</td> </tr> <tr> <td>USER_PROPS_ID</td>     <td>Код профиля
	* покупателя.</td> </tr> <tr> <td>ORDER_PROPS_ID</td>     <td>Код свойства заказа.</td> </tr>
	* <tr> <td>USER_VALUE_NAME</td>     <td>Название свойства заказа, сохраненное в
	* профиле покупателя.</td> </tr> <tr> <td>VALUE</td>     <td>Значение свойства
	* заказа, сохраненное в профиле покупателя.</td> </tr> <tr> <td>TYPE</td>    
	* <td>Тип свойства заказа.</td> </tr> <tr> <td>SORT</td>     <td>Индекс сортировки
	* свойства заказа.</td> </tr> <tr> <td>VARIANT_NAME</td>     <td>Название варианта
	* значения свойства заказа со значением из профиля покупателя.</td>
	* </tr> <tr> <td>CODE</td>     <td>Символьный код свойства заказа.</td> </tr>
	...
	*/
	...
	/**
	* <p>Метод добавляет новое свойство профиля покупателя в соответствии с массивом параметров arFields. Нестатический метод.</p>
	*
	*
