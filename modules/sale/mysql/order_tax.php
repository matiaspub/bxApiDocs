...
class CSaleOrderTax extends CAllSaleOrderTax
{
	...
	/**
	* <p>Метод возвращает результат выборки записей из налогов заказа в соответствии со своими параметрами. Нестатический метод.</p>
	*
	*
	...
	*
	* @param mixed $ASC  Массив, в соответствии с которым фильтруются 		записи налогов
	...
	* фильтра: 		<pre class="syntax">array("ORDER_ID" =&gt; 125)</pre> 		Этот фильтр означает
	* "выбрать все записи, в которых значение в поле ORDER_ID (код налога)
	* равно 125".<br><br> 	Значение по умолчанию - пустой массив array() -
	* означает, что результат отфильтрован не будет.
	*
	* @param array $arFilter = array() Массив полей, по которым группируются записи 		вариантов налогов
	...
	* будет.
	*
	* @param array $arGroupBy = false Массив параметров выборки. Может содержать следующие ключи: 		<ul>
	...
	*
	* @param array $arNavStartParams = false Массив полей записей, которые будут возвращены методом. Можно
	...
	* <th>Описание</th>   </tr> <tr> <td>ID</td>     <td>Код суммы налогов.</td> </tr> <tr>
	* <td>ORDER_ID</td>     <td>Код заказа.</td> </tr> <tr> <td>TAX_NAME</td>     <td>Название
	* налога.</td> </tr> <tr> <td>VALUE</td>     <td>Ставка налога.</td> </tr> <tr> <td>VALUE_MONEY</td>    
	* <td>Сумма налога.</td> </tr> <tr> <td>APPLY_ORDER</td>     <td>Порядок применения.</td>
	* </tr> <tr> <td>CODE</td>     <td>Символьный код налога.</td> </tr> <tr> <td>IS_IN_PRICE</td>    
	* <td>Флаг (Y/N) включен ли налог в цену товара.</td> </tr> <tr> <td>IS_PERCENT</td>    
	* <td>Y</td>   </tr> </table><p>Если в качестве параметра arGroupBy передается
	...
	*/
	...
	/**
	* <p>Метод добавляет новую сумму налога к заказу. Нестатический метод.</p>
	*
	*
	...
	* символьный код налога;</li> 	<li> <b>IS_PERCENT</b> - должно быть значение
