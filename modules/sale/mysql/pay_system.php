...
class CSalePaySystem extends CAllSalePaySystem
{
	...
	/**
	 * <p>Функция возвращает результат выборки записей из платежных систем в соответствии со своими параметрами.</p>
	 *
	 *
	 *
	 *
	 ...
	 *
	 *
	 * @param array $arFilter = array() Массив, в соответствии с которым фильтруются записи платежных
	 ...
	 *
	 *
	 * @param array $arGroupBy = false Массив полей, по которым группируются записи платежных систем.
	 ...
	 * функцию передается тип плательщика) </td> </tr> <tr> <td>PSA_NAME</td>
	 * <td>Название обработчика (возвращается, если в функцию передается
	 * тип плательщика)</td> </tr> <tr> <td>PSA_ACTION_FILE</td> <td>Скрипт обработчика
	 * (возвращается, если в функцию передается тип плательщика)</td> </tr>
	 * <tr> <td>PSA_RESULT_FILE</td> <td>Скрипт запроса результатов (возвращается, если
	 * в функцию передается тип плательщика)</td> </tr> <tr> <td>PSA_NEW_WINDOW</td>
	 * <td>Флаг (Y/N) открывать ли скрипт обработчика в новом окне
	 * (возвращается, если в функцию передается тип плательщика)</td> </tr>
	 * <tr> <td>PSA_PERSON_TYPE_ID</td> <td>Код типа плательщика.</td> </tr> <tr> <td>PSA_PARAMS</td>
	 * <td>Параметры вызова обработчика.</td> </tr> <tr> <td>PSA_HAVE_PAYMENT</td> <td>Есть
	 * вариант обработчика для работы после оформления заказа.</td> </tr> <tr>
	 * <td>PSA_HAVE_ACTION</td> <td>Есть вариант обработчика для мгновенного
	 * списания денег.</td> </tr> <tr> <td>PSA_HAVE_RESULT</td> <td>Есть скрипт запроса
	 * результатов.</td> </tr> <tr> <td>PSA_HAVE_PREPAY</td> <td>Есть вариант обработчика
	 * для работы во время оформления заказа.</td> </tr> </tbody></table><p>Если в
	...
	/**
	 * <p>Метод добавляет новую платежную систему на основании параметров из массива arFields. </p>
	 *
	 *
	 *
	 *
