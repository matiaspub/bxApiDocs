...
class CSaleOrder extends CAllSaleOrder
{
	...
	/**
	* <p>Метод добавляет новый заказ с параметрами из массива <i> arFields</i>. Перед добавлением заказа вызываются обработчики события OnBeforeOrderAdd модуля магазина, а после добавления - обработчики события OnOrderAdd модуля магазина. Нестатический метод.</p>
	*
	*
	...
	* заказа;</li> 	<li> <b>CURRENCY</b> - валюта стоимости заказа;</li> 	<li>
	* <b>DISCOUNT_VALUE</b> - общая величина скидки; <p class="note">Параметр не
	* поддерживается с версии 12.5.0.  Вместо него необходимо
	* использовать <a href="http://dev.1c-bitrix.ru/user_help/marketing/discounts/basket_rules.php"
	* >правила корзины</a>.</p> </li> 	<li> <b>USER_ID</b> - код пользователя
	...
	* посетитель;</li> 	<li> <b>STAT_GID</b> - параметр события в статистике;      </li>
	* 	<li> <b>PS_STATUS</b> - флаг (Y/N) статуса платежной системы - успешно ли
	* оплачен заказ (для платежных систем, которые позволяют
	* автоматически получать данные по проведенным через них
	* заказам);</li> 	<li> <b>PS_STATUS_CODE</b> - код статуса платежной системы
	* (значение зависит от системы);</li> 	<li> <b>PS_STATUS_DESCRIPTION</b> - описание
	* результата работы платежной системы;</li> 	<li> <b>PS_STATUS_MESSAGE</b> -
	* сообщение от платежной системы;</li> 	<li> <b>PS_SUM</b> - сумма, которая
	* была реально оплачена через платежную систему;</li> 	<li> <b>PS_CURRENCY</b> -
	* валюта суммы;</li> 	<li> <b>PS_RESPONSE_DATE</b> - дата получения статуса
	* платежной системы;</li> 	<li> <b>SUM_PAID </b> - сумма, которая уже была
	* оплачена покупателем по данному счету (например, с внутреннего
	* счета);</li> 	<li> <b>PAY_VOUCHER_NUM </b> - номер платежного 		поручения;</li> 	<li>
	* <b>PAY_VOUCHER_DATE</b> - дата платежного поручения.</li> </ul>
	*
	...
	*/
	...
	/**
	* <p>Метод изменяет параметры заказа с кодом ID на новые параметры из массива <i> arFields</i>. Перед добавлением заказа вызываются обработчики события OnBeforeOrderUpdate модуля магазина, а после добавления - обработчики события OnOrderUpdate модуля магазина. Нестатический метод.</p>
	*
	*
	...
	* системы;</li> 	<li> <b>PS_SUM</b> - сумма, которая была реально оплачена
	* через платежную систему;</li> 	<li> <b>PS_CURRENCY</b> - валюта суммы;</li> 	<li>
	...
	*
	* @param array $bDateUpdate  Обновление даты изменения заказа. Значения <i>true/false</i>
	...
	* 			<li> <b>@</b> - модификатор может использоваться при передаче набора
	* значений (массива); </li> 			<li> <b>%</b>  - значение поля проверяется на
	* соответствие передаваемой в фильтр строке в соответствии с
	* языком запросов.</li> 	</ul> В качестве "название_поляX" может стоять
	* любое поле 		заказов.<br><br> Для фильтрации заказов по свойствам
	* возможны следующие варианты:  <ul> <li> <b>PROPERTY_ID</b> - идентификатор
	* значения свойства заказа по идентификатору свойства заказа; </li>
	* <li> <b>PROPERTY_ORDER_PROPS_ID</b> -  идентификатор свойства заказа по
	...
	* "название_поля<i>N</i>" может стоять любое поле 		заказов. В качестве
	...
	* текущего сайта.</td>   </tr> <tr> <td>DATE_PAYED</td>     <td>Дата изменения флага
	* оплаты заказа в формате текущего сайта.</td>   </tr> <tr> <td>PS_RESPONSE_DATE</td>    
	* <td>Дата получения статуса платежной системы  в формате текущего
	* сайта.</td>   </tr> <tr> <td>DATE_ALLOW_DELIVERY</td>     <td>Дата изменения флага
	* разрешения доставки заказа в формате текущего сайта.</td> 	</tr> <tr>
	...
	* <td>PAY_VOUCHER_DATE</td>     <td>Дата платежного поручения.</td> 	</tr> </table><p>Если в
