...
class CSaleBasket extends CAllSaleBasket
{
	...
	*/
	...
	/**
	* <p>Метод возвращает результат выборки записей из корзины в соответствии со своими параметрами. Нестатический метод.</p>
	*
	*
	...
	* находится в передаваемом в фильтр массиве значений;</li> 			<li> <b>~</b>  -
	...
	* любое поле корзины.<br><br> 		Пример фильтра: 		<pre class="syntax">array("+&gt;=ORDER_ID"
	* =&gt; 20)</pre> 	Этот фильтр означает "выбрать все записи, в которых
	* значение в поле ORDER_ID больше или равно 20 либо не установлено (т.е.
	* null или ноль)".<br> 	Значение по умолчанию - пустой массив array() -
	* означает, что результат отфильтрован не будет. <br><br> В фильтре
	* также можно использовать параметр USER_ID (не путать с FUSER_ID),
	* передавая в него реальный идентификатор пользователя. Например
	* вот так: <pre class="syntax">'!USER_ID' =&gt; false</pre>
	*
	...
	* "название_поля<i>N</i>" может стоять любое поле корзины. В качестве
	...
	* width="15%">Ключ</th>     <th>Описание</th>   </tr> <tr> <td>ID</td>     <td>Код записи.</td> </tr>
	* <tr> <td>PRODUCT_ID</td>     <td>Уникальный в рамках модуля код товара.</td>   </tr>
	* <tr> <td>PRODUCT_PRICE_ID</td>     <td>Дополнительный код товара.</td>   </tr> <tr>
	* <td>PRICE</td>     <td>Стоимость единицы товара.</td>   </tr> <tr> <td>CURRENCY</td>    
	* <td>Валюта стоимости единицы товара.</td>   </tr> <tr> <td>WEIGHT</td>     <td>Вес
	* единицы товара.</td>   </tr> <tr> <td>QUANTITY</td>     <td>Количество единиц
	* товара.</td>   </tr> <tr> <td>LID</td>     <td>Сайт, на котором сделана покупка.</td>  
	* </tr> <tr> <td>DELAY</td>     <td>Флаг "товар отложен" (Y/N)</td>   </tr> <tr> <td>CAN_BUY</td>    
	* <td>Флаг "товар можно купить" (Y/N)</td>   </tr> <tr> <td>NAME</td>     <td>Название
	* товара.</td>   </tr> <tr> <td>CALLBACK_FUNC<font color="#FF0000">*</font> </td>     <td>Название
	* функции обратного вызова для поддержки актуальности корзины.</td>  
	* </tr> <tr> <td>MODULE</td>     <td>Модуль, добавляющий товар в корзину.</td>   </tr> <tr>
	* <td>NOTES</td>     <td>Особые заметки, например, тип цены.</td>   </tr> <tr>
	* <td>ORDER_CALLBACK_FUNC<font color="#FF0000">*</font> </td>     <td>Название функции обратного
	...
	* заказу.)</td>   </tr> <tr> <td>ORDER_CANCELED</td>     <td>Флаг отмены заказа.</td>   </tr> <tr>
	* <td>DETAIL_PAGE_URL</td>     <td>Ссылка на страницу детального просмотра
	* товара.</td>   </tr> <tr> <td>FUSER_ID</td>     <td>Внутренний код владельца корзины
	* (не совпадает с кодом пользователя) </td>   </tr> <tr> <td>USER_ID</td>    
	* <td>Реальный идентификатор пользователя. (не путать с FUSER_ID)</td>   </tr>
	* <tr> <td>ORDER_ID</td>     <td>Код заказа, в который вошла эта запись (товар).
	* Для товаров, которые помещены в корзину, но ещё не заказаны, это
	* поле равно NULL. </td>   </tr> <tr> <td>DATE_INSERT</td>     <td>Дата добавления товара в
	...
	*/
	...
	/**
	* <p>Метод возвращает результат выборки записей из свойств корзины в соответствии со своими параметрами. Нестатический метод.</p>
	*
	*
	* @param array $arOrder = array() Массив, в соответствии с которым сортируются результирующие
	* записи. Массив имеет вид: 		<pre class="syntax">array( "название_поля1" =&gt;
	...
	* модификатор может использоваться для целочисленных и
	* вещественных данных при передаче набора значений (массива). В
	* этом случае при генерации sql-запроса будет использован
	* sql-оператор IN, дающий компактную форму записи.</li> 			<li> <b>~</b>  -
	* значение поля проверяется на соответствие передаваемому в
	* фильтр шаблону;</li> 			<li> <b>%</b>  - значение поля проверяется на
	* соответствие передаваемой в фильтр строке в соответствии с
	* языком запросов.</li> 	</ul> В качестве "название_поляX" может стоять
	...
	* @return CDBResult <p>Возвращается объект класса <b>CDBResult</b>, содержащий набор
	* ассоциативных массивов с ключами: </p><table class="tnormal" width="100%"> <tr> <th
	* width="15%">Ключ</th>     <th>Описание</th>   </tr> <tr> <td>ID</td>     <td>Код записи. </td>  
	* </tr> <tr> <td>BASKET_ID</td>     <td>Код элемента корзины, к которому привязано
	* данное свойство.</td>   </tr> <tr> <td>NAME</td>     <td>Название свойства.</td>   </tr>
	* <tr> <td>VALUE</td>     <td>Значение свойства. </td>   </tr> <tr> <td>CODE</td>    
	* <td>Символьный код свойства. </td>   </tr> <tr> <td>SORT</td>     <td>Индекс
	* сортировки свойства. </td>   </tr> </table><p>Если в качестве параметра
	...
	*/
	...
	/**
	* <p>Метод добавляет товар в корзину, если его ещё нет, и обновляет параметры товара с увеличением количества, если он уже находится в корзине. В массиве <b>arFields</b> перечисляются все параметры товара, которые нужны для работы модуля Интернет-магазина (т.е. этот модуль не зависит от других модулей и работает полностью самостоятельно). Нестатический метод.</p>   <p>Интернет-магазин не зависит от других модулей, поэтому товары в корзину модуля продаж могут добавляться из любого места (например, из торгового каталога или со статической страницы). Для некоторых модулей существуют функции - оболочки, облегчающие добавление товара в корзину (например, для модуля <b>catalog</b> существуют функции <b>Add2Basket</b> и <b>Add2BasketByProductID</b>). </p>
	*
	*
	* @param array $arFields  Ассоциативный массив параметров элемента корзины, содержащий
	* следующие ключи: 		          <ul> <li> <b>PRODUCT_ID</b> - уникальный в рамках
	* модуля код товара (обязательное поле);</li>          			            <li>
	* <b>PRODUCT_PRICE_ID</b> - ID (идентификатор) конкретного ценового предложения
	* товара, пришедшего в корзину. Может быть использован в классе <a
	* href="http://dev.1c-bitrix.ru/api_help/catalog/classes/cprice/index.php">CPrice</a> модуля Catalog для
	* получения детальной информации о цене.</li>          			            <li> 		<b> 	
	* PRICE</b> - стоимость единицы товара (обязательное поле);</li>          			       
	*     <li> <b>CURRENCY</b> - валюта стоимости единицы товара (обязательное
	* поле), если валюта отличается от базовой валюты для данного сайта,
	* то стоимость будет автоматически сконвертирована по текущему
	* курсу;</li>          			            <li> <b>WEIGHT</b> - вес единицы товара;</li>          			     
	*       <li> <b>QUANTITY</b> - количество единиц товара;</li>          			            <li>
	* <b>LID</b> - сайт, на котором сделана покупка (обязательное поле);</li>      
	*    			            <li> <b>DELAY</b> - флаг "товар отложен" (Y/N);</li>          			            <li>
	* <b>CAN_BUY</b> - флаг "товар можно купить" (Y/N) - может устанавливаться
	* автоматически про наличии функции обратного вызова для
	* поддержки актуальности корзины;</li>          			            <li> <b>NAME</b> -
	* название товара (обязательное поле);</li>           <li> <b>PRODUCT_XML_ID</b> -
	* внешний код товара (необходим при обмене заказами с 1С);</li>           <li>
	* <b>CATALOG_XML_ID</b> - внешний код каталога (необходим при обмене заказами
	* с 1С);</li>           <li> <b>ORDER_ID</b> - идентификатор заказа. Ключ будет
	* пустым, если товар еще не добавлен в заказ;</li>          			            <li>
	* <b>CALLBACK_FUNC<font color="#FF0000">*</font></b> - название функции обратного вызова
	* для поддержки актуальности корзины (описание ниже);</li>          			        
	*    <li> <b>MODULE</b> - модуль, добавляющий товар в корзину;</li>          			           
	* <li> <b>NOTES</b> - особые заметки, например, тип цены;</li>          			            <li>
	* <b>ORDER_CALLBACK_FUNC<font color="#FF0000">*</font></b> - название функции обратного
	* вызова для оформления заказа (описание ниже);</li>          			            <li>
	* <b>DETAIL_PAGE_URL</b> - ссылка на страницу детального просмотра товара;</li>  
	*        			            <li> <b>CANCEL_CALLBACK_FUNC<font color="#FF0000">*</font></b> - название функции
	* обратного вызова для отмены заказа (описание ниже);</li>          			         
	*   <li> <b>PAY_CALLBACK_FUNC<font color="#FF0000">*</font></b> - название функции обратного
	* вызова, которая вызывается при установке флага "Доставка
	* разрешена" заказа;</li>           <li> <b>FUSER_ID</b> - идентификатор
	* пользователя интернет-магазина, необязательный параметр, по
	* умолчанию CSaleBasket::GetBasketUserID() (текущий пользователь);</li>          			          
	*  <li> <b>DISCOUNT_PRICE</b> - величина скидки;</li>           <li> <b>DISCOUNT_NAME</b> - название
	* скидки;</li>           <li> <b>DISCOUNT_VALUE</b> - размер скидки (в процентах);</li>       
	*    <li> <b>DISCOUNT_COUPON</b> - купон скидки;</li>           <li> <b>PROPS</b> - массив свойств
	* товара, который сохраняется в корзине. Каждый элемент этого
	* массива является массивом следующего формата: 		              <pre
	* class="syntax"><code>array("NAME" =&gt; "Название свойства",       "CODE" =&gt; "Код
	* свойства",        "VALUE" =&gt; "Значение свойства",        "SORT" =&gt; "Индекс
	* сортировки")</code></pre>            </li>           <li> <b>PRODUCT_PROVIDER_CLASS<font
	...
	*/
	...
	/**
	* <p>Метод удаляет запись корзины с кодом ID. Нестатический метод.</p> <a name="examples"></a>
	*
	*
	* @param mixed $intID  
	*
	...
	*/
	...
	/**
	* <p>Метод удаляет из корзины все записи с внутренним кодом владельца fUserID. Нестатический метод.</p>
	*
	*
	* @param int $fUserID  Внутренний код владельца.
	*
	* @param bool $bIncOrdered = false] Если флаг равен false (по-умолчанию), то удаляются только записи из
