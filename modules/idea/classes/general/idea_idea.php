...
Class CIdeaManagmentIdea
{
    ...
    public function GetCategoryList($CategoryIB = false)
    {
        ...
        if($obCache->StartDataCache($life_time, $cache_id, $cache_path))
        {
            ...
            $obSec = CIBlockSection::GetList(array("left_margin"=>"ASC"), array("IBLOCK_ID" => $CategoryIB));
    ...
    {   
        ...
        if($obCache->StartDataCache($life_time, $cache_id, $cache_path))
        {
            $arStatusField = CUserTypeEntity::GetList(
                array(),
                array("FIELD_NAME" => CIdeaManagment::UFStatusField)
            )->Fetch();
            if($arStatusField)
            {   
    ...
    //%TODO%
    static public function BindDublicate(){}
    //%TODO%
    static public function UnBindDublicate(){}
}
?>