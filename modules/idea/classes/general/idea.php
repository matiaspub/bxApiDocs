...
Class CIdeaManagment
{
    ...
    {
       return new CIdeaManagmentIdea($IdeaId);
    }
    //xAlias
    ...
    public function GetRSS($BlogCode, $type = "rss2.0", $numPosts = 10, $siteID = SITE_ID, $arPathTemplates = Array(), $arFilterExt = array())
    {
        ...
        //echo '<pre>'; print_r($arSettings); echo '</pre>'; die();
        ...
        if(!empty($arPathTemplates) && strlen($arPathTemplates["CUSTOM_TITLE"])>0)
            $arSettings["BLOG_NAME"] = htmlspecialchars($arPathTemplates["CUSTOM_TITLE"]);
        else
            $arSettings["BLOG_NAME"] = "\"".htmlspecialchars($arSite["NAME"])."\" (".$arSettings["SERVER_NAME"].")";
        ...
        {
            $atomID = "tag:".htmlspecialchars($arSettings["SERVER_NAME"]).",".date("Y-m-d");
        ...
        while($arPost = $dbPosts->Fetch())
        {
            ...
            $author = CBlogUser::GetUserName($arPost["BLOG_USER_ALIAS"], $arAuthorUser["NAME"], $arAuthorUser["LAST_NAME"], $arAuthorUser["LOGIN"]);
            ...
            if(!empty($arPathTemplates))
                $url = htmlspecialchars("http://".$arSettings["SERVER_NAME"].CComponentEngine::MakePathFromTemplate($arPathTemplates["BLOG_POST"], array("blog" => $arPost["BLOG_URL"], "post_id" => CBlogPost::GetPostID($arPost["ID"], $arPost["CODE"], $arPathTemplates["ALLOW_POST_CODE"]), "user_id"=>$arPost["BLOG_OWNER_ID"], "group_id"=>$arPost["BLOG_SOCNET_GROUP_ID"])));
            else
                $url = htmlspecialchars("http://".$arSettings["SERVER_NAME"].CBlogPost::PreparePath(htmlspecialchars($arPost["BLOG_URL"]), $arPost["ID"], $arPost["BLOG_GROUP_SITE_ID"]));

            $category = "";
            if(isset($arPost[self::UFCategroryCodeField]) && is_array($arSettings["CATEGORIES"][ToUpper($arPost[self::UFCategroryCodeField])]))
                $category = htmlspecialchars($arSettings["CATEGORIES"][ToUpper($arPost[self::UFCategroryCodeField])]["NAME"]);

            if(strlen($arPathTemplates["USER"]) > 0)
                $authorURL = htmlspecialchars("http://".$arSettings["SERVER_NAME"].CComponentEngine::MakePathFromTemplate($arPathTemplates["USER"], array("user_id"=>$arPost["AUTHOR_ID"], "group_id"=>$arPost["BLOG_SOCNET_GROUP_ID"])));
            else
                $authorURL = htmlspecialchars("http://".$arSettings["SERVER_NAME"].CBlogUser::PreparePath($arPost["AUTHOR_ID"], $arPost["BLOG_GROUP_SITE_ID"]));
            ...
            elseif ($arSettings["RSS_TYPE"] == "atom.03")
            {
                $atomID = "tag:".htmlspecialchars($arSettings["SERVER_NAME"]).":".$arBlog["URL"]."/".$arPost["ID"];
                ...
                $arSettings["RSS"] .= "    <name>".htmlspecialchars($author)."</name>\n";
