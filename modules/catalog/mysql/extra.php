...
class CExtra extends CAllExtra
{
	...
	/**
	* <p>Добавляет новую запись в таблицу наценок. Метод статический.</p>
	*
	*
	...
	*/
	...
	/**
	* <p>Метод возвращает список наценок в соответствии с фильтром и условиями сортировки. Метод статический.</p> <p></p> <div class="note"> <b>Примечание:</b> в таком виде метод работает с версии <b>11.0.0</b>. До этой версии использовалась устаревшая форма вызова метода (см. <a href="#old">ниже</a>).</div>
	*
	*
	* @param array $arOrder = array() Массив вида array(by1=&gt;order1[, by2=&gt;order2 [, ..]]), где by - поле для сортировки,
	* может принимать значения: 		         <ul> <li> <b>ID</b> - код (ID) наценки</li>        
	*  			           <li> <b>NAME</b> - название наценки</li>          			           <li> <b>PERCENTAGE</b> -
	* величина наценки</li>          		</ul>        		поле order - направление
	...
	* отбрасываются.         <br>        		Допустимыми являются следующие
	* модификаторы: 		         <ul> <li> <b>!</b> - отрицание;</li>          			           <li> <b>+</b>
	* - значения null, 0 и пустая строка так же удовлетворяют условиям
	* фильтра.</li>          		</ul>        		Допустимыми являются следующие
	...
	*          		</ul>        		"название поля" может принимать значения: 		         <ul>
	* <li> <b>ID</b> - код (ID) наценки (число)</li>          		           <li> <b>NAME</b> - название
	* наценки (строка)</li>          		           <li> <b>PERCENTAGE</b> - величина наценки
	* (число)</li>          		</ul>        		Значения фильтра - одиночное значение или
	* массив значений.         <br>        		Необязательное. По умолчанию
	* наценки не фильтруются.
	*
	* @param mixed $arGroupBy = false Массив полей для группировки наценок. Имеет вид: 		         <pre
	* class="syntax">array("название_поля1", "название_поля2", . . .)</pre>        		В
	* качестве "название_поля<i>N</i>" может стоять любое поле каталога.
	* <br><br>        		Если массив пустой, то метод вернет число записей,
	* удовлетворяющих фильтру. 		         <br>        		Значение по умолчанию -
	* <i>false</i> - означает, что результат группироваться не будет.
	*
	* @param mixed $arNavStartParams = false Массив параметров выборки. Может содержать следующие ключи: 			      
	...
	* параметра.</li>          			</ul>        			Необязательный. По умолчанию false -
	* наценки не ограничиваются.
	*
	...
	* class="tnormal" width="100%"><tbody> <tr> <th width="15%">Ключ</th>     <th>Описание</th>   </tr> <tr>
	* <td>ID</td>     <td>Код наценки.</td> </tr> <tr> <td>NAME</td>     <td>Название наценки.</td>
	* </tr> <tr> <td>PERCENTAGE</td>     <td>Величина наценки.</td>   </tr>
	* </tbody></table><h4>Примечания</h4><a name="old"></a><p>Сохранен старый способ
	* вызова:</p><pre class="syntax"><b>CDBResult CExtra::GetList(</b>  string by,   string order
	* <b>);</b></pre><p>где by - поле сортировки, а order - направление.</p><br><br>
	*
	* @static
