...
class CPrice extends CAllPrice
{
	...
	/**
	* <p>Метод добавляет новое ценовое предложение (новую цену) для товара. Нестатический метод.</p>
	*
	*
	* @param array $arFields  Ассоциативный массив параметров ценового предложения.
	* Допустимые параметры:          <ul> <li> <b>PRODUCT_ID </b> - код товара или
	* торгового предложения (ID элемента инфоблока).;</li>          	           <li>
	* <b>EXTRA_ID</b> - код наценки;</li>          	           <li> <b>CATALOG_GROUP_ID</b> - код типа
	* цены;</li>          	           <li> <b>PRICE</b> - цена;</li>          	           <li> <b>CURRENCY</b> -
	* валюта цены (обязательный параметр);</li>          	           <li> <b>QUANTITY_FROM</b> -
	* количество товара, начиная с приобретения которого действует эта
	* цена;</li>          	           <li> <b>QUANTITY_TO</b> - количество товара, при
	* приобретении которого заканчивает действие эта цена. 	             <p></p>
	* <div class="note"> <b>Примечание:</b> если необходимо, чтобы значения
	* параметров <b>QUANTITY_FROM</b> и <b>QUANTITY_TO</b> не были заданы, необходимо
	* указать у них в качестве значения false либо не задавать поля
	* <b>QUANTITY_FROM</b> и <b>QUANTITY_TO</b> в Update вообще.</div>             	</li>          	 	 </ul>       
	...
	* пересчета цен.         <br>        	Если добавляется базовая цена (в
	...
	* код наценки.         <br>        	Если добавляется иная цена (не базовая),
	...
	* <a href="http://dev.1c-bitrix.ru/api_help/catalog/events/onbeforepriceadd.php">OnBeforePriceAdd</a> и <a
	...
	* <li><a href="http://dev.1c-bitrix.ru/api_help/catalog/classes/cprice/update.php">CPrice::Update</a></li>  	   <li><a
	* href="http://dev.1c-bitrix.ru/api_help/catalog/events/onbeforepriceadd.php">Событие OnBeforePriceAdd</a></li>  	 
	*  <li><a href="http://dev.1c-bitrix.ru/api_help/catalog/events/onpriceadd.php">Событие OnPriceAdd</a></li> 
	...
	*/
	...
	/**
	* <p>Метод возвращает ценовое предложение по его коду ID. Нестатический метод. </p>
	*
	*
	...
	* <td>Код наценки.</td>   </tr> <tr> <td>CATALOG_GROUP_ID</td>     <td>Код типа цены. </td>   </tr>
	* <tr> <td>PRICE</td>     <td>Цена.</td>   </tr> <tr> <td>CURRENCY</td>     <td>Валюта.</td>   </tr> <tr>
	* <td>CAN_ACCESS</td>     <td>Флаг (Y/N), может ли текущий пользователь видеть эту
	* цену. </td>   </tr> <tr> <td>CAN_BUY</td>     <td>Флаг (Y/N), может ли текущий
	* пользователь покупать по этой цене.</td>   </tr> <tr> <td>CATALOG_GROUP_NAME</td>    
	* <td>Название группы цен на текущем языке. </td> 	</tr> <tr> <td>TIMESTAMP_X</td>     <td>
	* Дата последнего изменения записи. </td>   </tr> <tr> <td>QUANTITY_FROM </td>    
	* <td>Количество товара, начиная с приобретения которого действует
	* эта цена. </td>   </tr> <tr> <td>QUANTITY_TO </td>     <td>Количество товара, при
	* приобретении которого заканчивает действие эта цена. </td>   </tr>
	...
	*/
	...
	/**
	* <p>Метод возвращает результат выборки записей цен в соответствии со своими параметрами. Нестатический метод.</p> <p></p> <div class="note"> <b>Примечание</b>: Если выборку нужно произвести без учёта прав доступа, то лучше использовать метод <a href="http://dev.1c-bitrix.ru/api_help/catalog/classes/cprice/cprice__getlistex.8f7c2a3d.php">GetListEx</a>.</div>
	*
	*
	...
	* "название_поляX" может стоять любое поле типов 	цены.<br><br> 		Пример
	* фильтра: 	<pre class="syntax">array("PRODUCT_ID" =&gt; 150)</pre> 		Этот фильтр означает
	* "выбрать все записи, в которых значение в поле PRODUCT_ID (код товара)
	* равно 150".<br><br> 	Значение по умолчанию - пустой массив array() -
	...
	* 	В качестве "название_поля<i>N</i>" может стоять любое поле типов
	* 	цены. <br><br> 	Если массив пустой, то метод вернет число записей,
	...
	* наценки.</td> </tr> <tr> <td>CATALOG_GROUP_ID</td>     <td>Код типа цены.</td> </tr> <tr>
	* <td>PRICE</td>     <td>Цена.</td> </tr> <tr> <td>CURRENCY</td>     <td>Валюта.</td> </tr> <tr>
	* <td>CAN_ACCESS</td>     <td>Флаг (Y/N), может ли текущий пользователь видеть эту
	* цену.</td> </tr> <tr> <td>CAN_BUY</td>     <td>Флаг (Y/N), может ли текущий
	* пользователь покупать по этой цене.</td> </tr> <tr> <td>CATALOG_GROUP_NAME</td>    
	* <td>Название группы цен на текущем языке.</td> </tr> <tr> <td>TIMESTAMP_X</td>     <td>
	...
	*/
	...
	/**
	* <p>Метод возвращает результат выборки записей цен в соответствии со своими параметрами. Отличается от обычного <a href="http://dev.1c-bitrix.ru/api_help/catalog/classes/cprice/cprice__getlist.8f7c2a3e.php">GetList</a> отсутствием безусловной проверкой прав на типы цен для групп текущего пользователя. Метод необходимо использовать везде, где эта проверка не требуется (будет гораздо производительнее). Нестатический метод. </p>
	*
	*
	* @param array $arOrder = array() Массив, в соответствии с которым сортируются результирующие
	* записи. Массив имеет вид: 		<pre class="syntax">array( "название_поля1" =&gt;
	...
	* наценки.</td> </tr> <tr> <td>CATALOG_GROUP_ID</td>     <td>Код типа цены.</td> </tr> <tr>
	* <td>PRICE</td>     <td>Цена.</td> </tr> <tr> <td>CURRENCY</td>     <td>Валюта.</td> </tr> <tr>
	* <td>TIMESTAMP_X</td>     <td> Дата последнего изменения записи. </td> 	</tr> <tr>
	* <td>QUANTITY_FROM </td>     <td>Количество товара, начиная с приобретения
	* которого действует эта цена. </td> 	</tr> <tr> <td>QUANTITY_TO </td>    
	* <td>Количество товара, при приобретении которого заканчивает
	* действие эта цена.</td> 	</tr> <tr> <td>CATALOG_GROUP_BASE</td>     <td>Флаг "Базовая"
	* типа цены.</td> </tr> <tr> <td>CATALOG_GROUP_NAME</td>     <td>Название группы цен на
	* текущем языке.</td> </tr> <tr> <td>CATALOG_GROUP_SORT</td>     <td>Индекс сортировки
	* типа цены.</td> </tr> <tr> <td>GROUP_BUY</td>     <td>Флаг "Разрешена покупка по этой
	* цене"</td>   </tr> </table><p> Если в качестве параметра arGroupBy передается
