...
class CAllCatalogProductGroups
{
	...
	* товара;</li> 			<li> <b>GROUP_ID</b> - код группы пользователей;</li> 			<li>
	* <b>ACCESS_LENGTH</b> - длина периода, на который пользователь
	* привязывается к группе пользователей при покупке товара (0 -
	* навсегда);</li> 			<li> <b>ACCESS_LENGTH_TYPE</b> - тип периода, на который
	...
	*
	* @param int $intID = 0 Код записи с информацией о связи товаров и групп пользователей, к
	...
	*/
	...
	/**
	* <p>Метод выбирает параметры информации о связи между товаром и группами пользователей по коду информации ID. Нестатический метод.</p>
	*
	*
	...
	*/
	...
	/**
	* <p>Метод изменяет параметры записи с кодом ID информации о связи товаров и групп пользователей, к которым пользователь привязывается при покупке товара, в соответствии с данными из массива arFields. Нестатический метод.</p>
	*
	*
	* @param mixed $intID  код записи 
	*
	...
	* Допустимые ключи: 		<ul> <li> <b> PRODUCT_ID</b> - код товара;</li> 			<li> <b> GROUP_ID</b> -
	* код группы пользователей;</li> 			<li> <b> ACCESS_LENGTH</b> - длина периода, на
	* который пользователь привязывается к группе пользователей при
	* покупке товара (0 - навсегда);</li> 			<li> <b> ACCESS_LENGTH_TYPE</b> - тип периода,
	* на который пользователь привязывается к группе пользователей
	* при покупке товара 				("H" - час, "D" - сутки, "W" - неделя, 				"M" - месяц, "Q" -
	* квартал, 				"S" - полугодие, "Y" - год).</li> 		</ul>
	*
	...
	*/
	...
	/**
	* <p>Метод удаляет информацию о связи товаров и группы пользователей, к которой пользователь привязывается при покупке товара. Нестатический метод.</p>
	*
	*
	...
	*/
	...
	/**
	* <p>Метод удаляет привязку к группе пользователей с кодом <i>ID</i> из всех карточек товаров-подписок (продажа прав). Нестатический метод.</p>
	*
	*
