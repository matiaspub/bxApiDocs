...
/*	$strDiscountVat = (!empty($_REQUEST['discount_vat']) && $_REQUEST['discount_vat'] == 'N' ? 'N' : 'Y');
	COption::SetOptionString('catalog', 'discount_vat', $strDiscountVat); */
		...
		foreach ($arNewIBlocksList as $intIBlockID => $arIBlockInfo)
		{
			...
			{
				// double
				if (in_array($arIBlockInfo['OFFERS_IBLOCK_ID'],$arOffersIBlocks))
				{
					$boolFlag = false;
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['agent_start']) && strlen($_POST['agent_start']) > 0 && !$bReadOnly && check_bitrix_sessid())
	{
		CAgent::RemoveAgent('CCatalog::PreGenerateXML("yandex");', 'catalog');
	...
	$strEnableReservation = COption::GetOptionString('catalog','enable_reservation','Y');
	...
	$arCatalog = array();
	while ($arCatalogs = $dbCatalog->Fetch())
	{
		$arCatalog[] = $arCatalogs;
	}
	$arStores = array();
...
<?
	...
	while ($arIBlock = $rsIBlocks->Fetch())
	{
		...
		if (!array_key_exists($arIBlock['ID'],$arIBlockSitesList))
		{
			$arLIDList = array();
			$arWithLinks = array();
...
<table width="100%" cellspacing="0" cellpadding="0" border="0" class="internal">
	...
	foreach ($arIBlockFullInfo as &$res)
	{
		...
		<tr>
			...
			<td align="center"><select id="OFFERS_IBLOCK_ID_<? echo $res["ID"]; ?>" name="OFFERS_IBLOCK_ID_<? echo $res["ID"]; ?>" class="typeselect" <? if ('Y' == $res['IS_OFFERS']) { ?>disabled="disabled"<? } else { ?>onchange="show_add_offers(<? echo $res["ID"]; ?>,this);"<? } ?> style="width: 100%;">
...
<?
	...
	<tr>
		...
		<td>
			<select style='max-width:300px' id="catalogs_id" name="catalogs_id" <?=($bReadOnly) ? " disabled" : ""?>>
				...
				{
					echo"<option value=".$catalog['ID'].">".$catalog["NAME"]." (".$catalog["LID"].")"."</option>";
				}
				?>
			</select>
		</td>
	...
	<tr>
		...
		<td width="60%">
			<input type="button" <?if($disableClearButtons) echo " disabled";?> value="<? echo GetMessage("CAT_CLEAR_ACTION"); ?>" id="cat_clear_quantity_btn" onclick="catClearQuantity(this, 'clearQuantity')" title="">
	...
	<tr>
		...
		<td>
			<input type="button" <?if($disableClearButtons) echo " disabled";?> value="<? echo GetMessage("CAT_CLEAR_ACTION"); ?>" id="cat_clear_reserved_quantity_btn" onclick="catClearQuantity(this, 'clearReservedQuantity')" title="">
	...
	<tr>
		...
		<td>
			<input type="button" <?if($disableClearButtons) echo " disabled";?> value="<? echo GetMessage("CAT_CLEAR_ACTION"); ?>" id="cat_clear_store_btn" onclick="catClearQuantity(this, 'clearStore')" title="">
