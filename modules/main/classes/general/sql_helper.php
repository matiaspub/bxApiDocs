...
class CSqlHelper
{
	...
	public static function GetRoughCount(&$arSql, $tableName, $tableAlias = '', $dbType = '')
	{
		global $DB;
		...
		if($dbType !== 'MYSQL')
		{
			return self::GetRowCount($arSql, $tableName, $tableAlias, $dbType);
		}

		$query = 'EXPLAIN SELECT COUNT(\'x\') as CNT FROM '.$tableName;
		...
		$tableName = $tableAlias !== '' ? $tableAlias : $tableName;
		$rs = $DB->Query($query, false, 'File: '.__FILE__.'<br/>Line: '.__LINE__);
		while($ary = $rs->Fetch())
		{
			$currentTableName = isset($ary['table']) ? $ary['table'] : '';
			if($currentTableName === $tableName && isset($ary['rows']))
			{
				return intval($ary['rows']);
			}
		}
