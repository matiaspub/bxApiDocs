...
/**
 * <p>Инициализирует, либо запоминает переменные фильтра в сессии.</p> <p class="note">Функция работает с переменными из глобальной области видимости, это необходимо учитывать при [link=89638#make]создании основных файлов компонентов[/link].</p>
 *
 *
 *
 *
...
/**
 * <p>Очищает переменные, содержащие значения фильтра, и очищает соответствующие сессионные переменные.</p> <p class="note">Функция работает с переменными из глобальной области видимости, это необходимо учитывать при [link=89638#make]создании основных файлов компонентов[/link].</p>
 *
 *
 *
 *
 ...
 *
 *
 * @param array $extra_vars = array("filter"=>"Y" В данном параметре можно передать массив, представляющий из себя
 ...
 *
 *
 * @param bool $safe_for_html = true Если значение равно "true", то результат будет приведен в
 * HTML-безопасный вид.
 *
 *
 *
...
/**
 * <p>Инициализирует параметры сортировки хранимые в сессии, если сортировка не была явно задана. Если сортировка задана явно, то функция запоминает параметры сортировки в сессии.</p> <p class="note">Функция работает с переменными из глобальной области видимости, это необходимо учитывать при [link=89638#make]создании основных файлов компонентов[/link].</p>
 *
 *
 *
 *
 * @param mixed $page = false Для какой страницы инициализировать сортировку. Значение "false"
 ...
 *
 *
 * @param mixed $page = false URL страницы вместе со всеми параметрами для которой будет
