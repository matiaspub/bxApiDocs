...
class CAdv extends CAllAdv
{
	...
	public static function GetList(&$by, &$order, $arFilter=Array(), &$is_filtered, $limit="", &$arrGROUP_DAYS, &$strSql_res)
	{
		...
		if ($find_group=="NOT_REF") $arrFields = array_merge($arrFields, $arrFields_2);
		if ($order!="asc") $order = "desc";
		$key = array_search(strtoupper($by),$arrFields);
		if ($key===NULL || $key===false) $key = array_search("A.".strtoupper($by),$arrFields);
		if ($key!==NULL && $key!==false) $strSqlOrder = " ORDER BY ".$arrFields[$key];
		elseif ($by == "s_dropdown") $strSqlOrder = "ORDER BY A.ID desc, A.REFERER1, A.REFERER2";
		elseif ($by == "s_referers") $strSqlOrder = "ORDER BY A.REFERER1, A.REFERER2";
		else
		{
		...
		{
			$group = true;
			if ($find_group=="referer1") $group = "REFERER1"; else $group = "REFERER2";
	...
	public static function GetEventListByReferer($value, $arFilter)
	{
		$err_mess = "File: ".__FILE__."<br>Line: ";
		$DB = CDatabase::GetModuleConnection('statistic');
		if ($arFilter["GROUP"]=="referer1") $group = "A.REFERER1"; else $group = "A.REFERER2";
		if (is_array($arFilter))
		{
			$date1 = $arFilter["DATE1_PERIOD"];
			$date2 = $arFilter["DATE2_PERIOD"];
