...
class CAdvContract extends CAdvContract_all
{
	 ...
	 *
	 *
	 * @param varchar &$order  Массив для фильтрации значений. В массиве допустимы следующие
	 * индексы: <ul> <li>ID - ID контракта (допускается сложная логика) </li> <li>- "Y"
	 * - при фильтрации по ID контракта будет искаться точное совпадение
	 * (по умолчанию); "N" - в противном случае будет искаться вхождение </li>
	 * <li>LAMP - значение индикатора: "red", "green" </li> <li>DATE_MODIFY_1 - левая часть
	 * интервала для даты модификации контракта </li> <li>DATE_MODIFY_2 - правая
	 * часть интервала для даты модификации контракта </li> <li>NAME - имя
	 * контракта (допускается сложная логика) </li> <li>NAME_EXACT_MATCH - "Y" - при
	 * фильтрации по имени контракта будет искаться точное совпадение;
	 * "N" - в противном случае будет искаться вхождение (по умолчанию) </li>
	 * <li>DESCRIPTION - описание контракта (допускается сложная логика) </li>
...
class CAdvBanner extends CAdvBanner_all
{
	 ...
	 * индексы: <ul> <li>ID - ID баннера (допускается <a
	 * href="http://dev.1c-bitrix.ru/user_help/general/filter.php">сложная логика</a>) </li> <li>ID_EXACT_MATCH -
	 * "Y" - при фильтрации по ID баннера будет искаться точное совпадение
	 * (по умолчанию); "N" - в противном случае будет искаться вхождение </li>
	 * <li>ACTIVE - активность, значения Y/N</li> <li>LAMP - значение индикатора: "red",
	 * "green" </li> <li>LANG - двухсимвольный идентификатор языковой части сайта
	 * в которой будет показываться баннер </li> <li>SHOW_COUNT_1 - левая часть
	 * интервала для числа показов </li> <li>SHOW_COUNT_2 - правая часть интервала
	 * для числа показов </li> <li>CLICK_COUNT_1 - левая часть интервала для числа
	 * кликов </li> <li>CLICK_COUNT_2 - правая часть интервала для числа кликов </li>
	 * <li>CTR_1 - левая часть интервала для CTR баннера </li> <li>CTR_2 - правая часть
	 * интервала для CTR баннера </li> <li>GROUP - имя группы баннера
	 * (допускается <a href="http://dev.1c-bitrix.ru/user_help/general/filter.php">сложная логика</a>)
	 * </li> <li>GROUP_EXACT_MATCH - "Y" - при фильтрации по имени группы будет искаться
	 * точное совпадение; "N" - в противном случае будет искаться
	...
	/**
	 * <p>Функция фиксирует клик по баннеру в базе данных.</p>
	 *
	 *
	 *
	 *
	 ...
	 * контракта берется из параметров баннера.
	 *
	 *
	 *
	 * @return mixed 
