...
class CTicket extends CAllTicket
{
	 ...
	 * индексы: <ul> <li>ID - ID обращения (допускается сложная логика) </li>
	 * <li>ID_EXACT_MATCH - "Y" - при фильтрации по ID обращения будет искаться
	 * точное совпадение (по умолчанию); "N" - в противном случае будет
	 * искаться вхождение </li> <li>LID - ID сайта </li> <li>LID_EXACT_MATCH - "Y" - при
	 * фильтрации по ID языка будет искаться точное совпадение (по
	 * умолчанию); "N" - в противном случае будет искаться вхождение </li>
	 * <li>LAMP - массив индикаторов: "red", "yellow", "green", "green_s", "grey" </li> <li>DATE_CREATE_1 -
	 * левая часть интервала для даты создания обращения </li> <li>DATE_CREATE_2 -
	 * правая часть интервала для даты создания обращения </li>
	 * <li>DATE_TIMESTAMP_1 - левая часть интервала для даты модификации
	 ...
	 *
	 *
	 * @param char(1) $CHECK_RIGHTS = "Y" "Y" - будут выбраны только те обращения которые текущий
	 * пользователь может просматривать (по умолчанию); "N" - выбирать все
	 * обращения независимо от прав текущего пользователя.
	 *
	 *
	 *
	 * @param char(1) $get_user_name = "Y" "Y" - при выборке обращений будут также выбраны такие поля как
	 * OWNER_LOGIN, OWNER_NAME, RESPONSIBLE_LOGIN, RESPONSIBLE_NAME, MODIFIED_LOGIN, MODIFIED_NAME, LAST_MESSAGE_LOGIN,
	 * LAST_MESSAGE_NAME, CREATED_LOGIN, CREATED_EMAIL, CREATED_NAME, описывающие параметры
	 * пользователей имевших отношение к данному обращению; "N" -
	 * вышеперечисленные поля не будут выбраны, но зато это ускорит
	 * работу функции.
	 *
	 *
	 *
	 * @param char(1) $get_dictionary_name = "Y" "Y" - при выборке обращений будут также выбраны такие поля как
	 * CATEGORY_NAME, CATEGORY_SID, CRITICALITY_NAME, CRITICALITY_SID, STATUS_NAME, STATUS_SID, MARK_NAME, MARK_SID,
	 * SOURCE_NAME, SOURCE_SID, описывающие поля из справочника техподдержки; "N" -
	 * вышеперечисленные поля не будут выбраны, но зато это ускорит
	 * работу функции.
	 *
	 *
	 ...
	 * закрытия); по умолчанию - код текущего сайта.
	 *
	 *
	 *
	 * @return record 
	...
	/**
	 * <p>Функция предназначена для получения списка сообщений в одном обращении.</p>
	 *
	 *
	 *
	 *
	 ...
	 * <li>ID_EXACT_MATCH - "Y" - при фильтрации по ID сообщения будет искаться
	 * точное совпадение (по умолчанию); "N" - при фильтрации по ID
	 * сообщения будет искаться вхождение </li> <li>TICKET_ID - ID обращения
	 * (допускается сложная логика) </li> <li>TICKET_ID_EXACT_MATCH - "Y" - при фильтрации
	 * по ID обращения будет искаться точное совпадение (по умолчанию); "N"
	 * - при фильтрации по ID обращения будет искаться вхождение </li>
	 * <li>IS_HIDDEN - "Y" - будут выбраны только скрытые обращения; "N" - будут
	 * выбраны все не скрытые сообщения; </li> <li>IS_LOG - "Y" - будут выбраны
	 * только лог-записи (записи изменений); "N" - будут выбраны не
	 * лог-записи; </li> </ul>
	 *
	 *
	 *
	 ...
	 *
	 *
	 * @param char(1) $CHECK_RIGHTS = "Y" Флаг необходимости проверки прав текущего пользователя: <ul> <li>Y -
	 ...
	 * независимо от прав текущего пользователя </li> </ul>
	 *
	 *
	 *
	 ...
	 * данному сообщению; "N" - вышеперечисленные поля не будут выбраны,
	 * но зато это ускорит работу функции.
	 *
	 *
	 *
	 * @return record 
