...
class CFormResult extends CAllFormResult
{
	...
	/**
	* <p>Возвращает список <a href="http://dev.1c-bitrix.ru/api_help/form/terms.php#result">результатов</a> веб-формы в виде объекта класса <a href="http://dev.1c-bitrix.ru/api_help/main/reference/cdbresult/index.php">CDBResult</a>. Метод нестатический.</p>   <p class="note"><b>Примечание</b>    <br>  Возвращаемый список содержит только <a href="http://dev.1c-bitrix.ru/api_help/form/classes/cformresult/index.php">поля результата</a>. Значения ответов и полей можно получить с помощью метода <a href="http://dev.1c-bitrix.ru/api_help/form/classes/cform/getresultanswerarray.php">CForm::GetResultAnswerArray</a> или <a href="http://dev.1c-bitrix.ru/api_help/form/classes/cformresult/getdatabyid.php">CFormResult::GetDataByID</a>.</p>
	*
	*
	* @param int $form_id  ID веб-формы.
	*
	* @param string &$by = "s_timestamp" Ссылка на переменную с полем для сортировки; может принимать
	* значения: 	          <ul> <li> <b>s_id</b> - ID результата; 		</li>                     <li>
	* <b>s_date_create</b> - дата создания; 		</li>                     <li> <b>s_timestamp</b> - дата
	* изменения (значение по умолчанию); 		</li>                     <li> <b>s_user_id</b> - ID
	* пользователя, создавшего результат; 		</li>                     <li> <b>s_guest_id</b> -
	* ID посетителя, создавшего результат; 		</li>                     <li> <b>s_session_id</b> -
	* ID сессии, в которой был создан результат; 		</li>                     <li>
	* <b>s_status</b> - ID статуса. </li>          </ul>
	*
	* @param string &$order = "desc" Ссылка на переменную с порядком сортировки. Допустимы следующие
	* значения: 	          <ul> <li> <b>desc</b> - по убыванию (значение по умолчанию);
	* 		</li>                     <li> <b>asc</b> - по возрастанию. </li>          </ul>
	*
	* @param array $filter = array() Массив содержащий параметры фильтра. Необязательный параметр. В
	* массиве допустимы следующие ключи: 	          <ul> <li> <b>ID</b>* - ID
	* результата (по умолчанию будет искаться точное совпадение); 		</li>   
	*                  <li> <b>ID_EXACT_MATCH</b> - если значение равно "N", то при фильтрации
	* по <b>ID</b> будет искаться вхождение; 		</li>                     <li> <b>STATUS_ID</b>* - ID
	* статуса (по умолчанию будет искаться точное совпадение); 		</li>         
	*            <li> <b>STATUS_ID_EXACT_MATCH</b> - если значение равно "N", то при
	* фильтрации по <b>STATUS_ID</b> будет искаться вхождение; 		</li>                    
	* <li> <b>TIMESTAMP_1</b> - левое значение интервала ("с") по дате изменения
	* (задается в формате даты текущего сайта); 		</li>                     <li>
	* <b>TIMESTAMP_2</b> - правое значение интервала ("по") по дате изменения
	* (задается в формате даты текущего сайта); 		</li>                     <li>
	* <b>DATE_CREATE_1</b> - левое значение интервала ("с") по дате создания
	* (задается в формате даты текущего сайта); 		</li>                     <li>
	* <b>DATE_CREATE_2</b> - правое значение интервала ("по") по дате создания
	* (задается в формате даты текущего сайта); 		</li>                     <li>
	* <b>TIME_CREATE_1</b> - левое значение интервала ("с") по дате создания в
	* полном формате (дата и время); </li>                     <li> <b>TIME_CREATE_2</b> - правое
	* значение интервала ("по") по дате создания в полном формате (дата и
	* время);		 </li>                     <li> <b>REGISTERED</b> - флаг зарегистрированности
	* автора результата; допустимы следующие значения: 			              <ul> <li>
	* <b>Y</b> - автор был зарегистрирован как пользователь; 				</li>                  
	*           <li> <b>N</b> - автор не был зарегистрирован как пользователь. </li>  
	*            </ul> </li>                     <li> <b>USER_AUTH</b> - флаг авторизованности
	* автора результата; допустимы следующие значения: 			              <ul> <li>
	* <b>Y</b> - автор был авторизован; 				</li>                             <li> <b>N</b> - автор
	* не был авторизован. </li>              </ul> </li>                     <li> <b>USER_ID</b>* - ID
	* пользователя, создавшего результат (автор результата) (по
	* умолчанию будет искаться точное совпадение); 		</li>                     <li>
	* <b>USER_ID_EXACT_MATCH</b> - если значение равно "N", то при фильтрации по
	* <b>USER_ID</b> будет искаться вхождение; 		</li>                     <li> <b>GUEST_ID</b>* - ID
	* посетителя создавшего результат (автор результата) (по умолчанию
	* будет искаться точное совпадение); 		</li>                     <li>
	* <b>GUEST_ID_EXACT_MATCH</b> - если значение равно "N", то при фильтрации по
	* <b>GUEST_ID</b> будет искаться вхождение; 		</li>                     <li> <b>SESSION_ID</b>* - ID
	* сессии в которой был создан результат (по умолчанию будет
	* искаться точное совпадение); 		</li>                     <li> <b>SESSION_ID_EXACT_MATCH</b> -
	* если значение равно "N", то при фильтрации по <b>SESSION_ID</b> будет
	* искаться вхождение; 		</li>                     <li> <b>FIELDS</b> - массив, содержащий
	* параметры фильтра для фильтрации <a
	* href="http://dev.1c-bitrix.ru/api_help/form/terms.php#result">результатов</a> по значениям <a
	...
	* <b>CODE</b>;</li>                             <li> <b>PARAMETER_NAME</b> - тип данных по которым
	* фильтруем, допустимы следующие значения: 					                  <ul> <li> <b>USER</b>
	* - фильтруем по ответам введенных авторами с клавиатуры (по
	* умолчанию); 						</li>                                     <li> <b>ANSWER_TEXT</b> - фильтруем по
	* параметру <font color="green">ANSWER_TEXT</font>; 						</li>                                     <li>
	* <b>ANSWER_VALUE</b> - фильтруем по параметру <font color="red">ANSWER_VALUE.</font> </li>           
	*       </ul> </li>                             <li> <b>VALUE</b> - значение, по которому
	* фильтруем (допускается <a href="http://dev.1c-bitrix.ru/user_help/general/filter.php">сложная
	* логика</a>); 				</li>                             <li> <b>FILTER_TYPE</b> - тип фильтра,
	* определяет, как интерпретировать данные по которым фильтруем: 					
	*                  <ul> <li> <b>integer</b> - означает, что данные, по которым будет
	* осуществляться фильтрация, считать числами (используется только
	* с <b>PARAMETER_NAME</b>=[USER|ANSWER_TEXT|ANSWER_VALUE]); 						</li>                                     <li>
	* <b>text</b> - означает, что данные, по которым будет осуществляться
	* фильтрация, должны обрабатываться как текстовые поля
	* (используется только с <b>PARAMETER_NAME</b>=[USER|ANSWER_TEXT|ANSWER_VALUE]); 						</li>           
	*                          <li> <b>date</b> - означает, что данные по которым будет
	* осуществляться фильтрация, должны обрабатываться как даты
	* (используется только с <b>PARAMETER_NAME</b>=USER); 						</li>                                    
	* <li> <b>answer_id</b> - означает, что фильтрация будет производиться только
	* по прямому совпадению с ID <a
	* href="http://dev.1c-bitrix.ru/api_help/form/terms.php#answer">ответа</a> (при этом PARAMETER_NAME не
	* имеет значения). </li>                  </ul> </li>                             <li> <b>PART</b> - если
	* тип фильтра <b>FILTER_TYPE</b>=[integer|date], то данное поле
	* <b>должно</b>содержать одно из трех значений: 					                  <ul> <li> <b>0</b>
	* - прямое совпадение со значением; 						</li>                                     <li>
	* <b>1</b> - левое значение интервала ("с"); 						</li>                                     <li>
	* <b>2</b> - правое значение интервала ("по") </li>                  </ul> </li>                 
	*            <li> <b>EXACT_MATCH</b> - если <b>FILTER_TYPE</b>="text", то в данном поле можно
	* задать следующие значения: 					                  <ul> <li> <b>Y</b> - прямое
	* совпадение; 						</li>                                     <li> <b>N</b> - будет искаться
	* вхождение (по умолчанию). </li>                  </ul> </li>              </ul> </li>          </ul>
	*
	...
	* проверить; 		</li>                     <li> <b>N</b> - права не нужно проверять. </li>    
	*      </ul>        	Для того чтобы <a
	* href="http://dev.1c-bitrix.ru/api_help/form/terms.php#result">результат</a> попал в
	* результирующий список, необходимо обладать следующими <a
	* href="http://dev.1c-bitrix.ru/api_help/form/permissions.php">правами</a>: 	          <ol> <li>На
	* веб-форму <i>form_id</i>: 			              <br><br><b>[20] Работа со всеми результатами
	* в соответствии с их статусами</b> 			              <br><br>            или             
	* <br><br><b>[15] Работа со своим результатом в соответствии с его
	* статусом</b> - в этом случае результирующий список будет состоять
	* только из тех результатов создателем которых является текущий
	* пользователь. 			              <br> </li>                     <li>На статус, в котором
	* находится результат, необходимо иметь право: 			              <br><br><b>[VIEW]
	* просмотр</b> 			              <br><br>            или              <br><br><b>[EDIT]
	* редактирование</b> 			              <br><br>            или              <br><br><b>[DELETE]
	* удаление</b> </li>          </ol>        	Параметр необязательный. По умолчанию
