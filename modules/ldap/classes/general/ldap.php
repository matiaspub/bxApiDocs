...
class CLDAP
{
	...
	public function Bind($login, $password)
	{
		...
		{
			$login = $GLOBALS["APPLICATION"]->ConvertCharset($login, SITE_CHARSET, "utf-8");
			$password = $GLOBALS["APPLICATION"]->ConvertCharset($password, SITE_CHARSET, "utf-8");
		}
	...
	public function WorkAttr($values)
	{
		...
		{
			if($this->arFields["CONVERT_UTF8"]=="Y")
				return $GLOBALS["APPLICATION"]->ConvertCharset($values[0], "utf-8", SITE_CHARSET);
			return $values[0];
		}
		unset($values['count']);
		if($this->arFields["CONVERT_UTF8"]=="Y")
			foreach($values as $key=>$val)
				$values[$key] = $GLOBALS["APPLICATION"]->ConvertCharset($val, "utf-8", SITE_CHARSET);
	...
	public function QueryArray($str = '(ObjectClass=*)', $fields = false)
	{
		...
		foreach($arBaseDNs as $BaseDN)
		{
			...
			if($this->arFields["CONVERT_UTF8"]=="Y")
			{
				$BaseDN = $GLOBALS["APPLICATION"]->ConvertCharset($BaseDN, SITE_CHARSET, "utf-8");
	...
	public static function OnUserLogin($arArgs)
	{
		...
		$db_events = GetModuleEvents("ldap", "OnBeforeUserLogin");
	...
	public function GetUserFields($arLdapUser, &$departmentCache=FALSE)
	{
		...
		foreach($this->arFields["FIELD_MAP"] as $userField=>$attr)
		{
			...
			if(is_array($arRes[$userField]))
			{
				...
				else if (!empty($arRes[$userField]['SETTINGS']['DEFAULT_VALUE']))
				{
	...
	public function SetUser($arLdapUser, $bAddNew = true)
	{
		...
		if ($ID > 0)
		{
			...
			$arUserBitrixGroups = array_merge($arUserBitrixGroups, $arUserBitrixGroupsNew);
