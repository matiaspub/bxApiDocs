...
class CSaleRecurring extends CAllSaleRecurring
{
	...
	/**
	 * <p>Метод возвращает параметры информации о продлении с кодом ID.</p>
	 *
	 *
	 *
	 *
	 ...
	 *
	 *
	 * @return bool <p>Метод возвращает ассоциативный массив параметров информации о
	 ...
	 * последнего изменения записи.</li> </ul><h4>Замечание</h4><p>Результат
	 * выполнения метода кешируется в рамках одной страницы. Поэтому
	 * повторные вызовы метода не влекут за собой дополнительных
	...
	/**
	 * <p>Функция возвращает результат выборки записей информации и продлении подписки в соответствии со своими параметрами.</p>
	 *
	 *
	 *
	 *
	 ...
	 * </ul> В качестве "название_поляX" может стоять любое поле информации
	 * о продлении подписки.<br><br> Пример фильтра: <pre class="syntax">array("USER_ID" =&gt;
	 * 150)</pre> Этот фильтр означает "выбрать все записи, в которых
	 * значение в поле USER_ID (код пользователя) равно 150".<br><br> Значение по
	 ...
	 * качестве параметра <i> arGroupBy</i> передается пустой массив, то
	...
	/**
	 * <p>Метод добавляет новую запись на продление подписки в соответствии с параметрами из массива arFields.</p>
	 *
	 *
	 *
	 *
